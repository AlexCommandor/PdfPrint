...
Public Sub SetOrientation(ByVal hwnd As Long, lDuplex As Long, iOrient As Integer, iPaperSize As Integer)
    ...
    MyDevMode.dmDuplex = lDuplex
    MyDevMode.dmOrientation = iOrient
    MyDevMode.dmPaperSize = iPaperSize
...
Public Sub AddHorScrollToList(ByRef oList As ListBox)
  Static x As Long, Item As Long
  For Item = oList.ListCount - 1 To 0 Step -1
    If x < oList.Parent.TextWidth(oList.List(Item) & " ") Then
      x = oList.Parent.TextWidth(oList.List(Item) & " ")
    End If
  Next
  If oList.Parent.ScaleMode = vbTwips Then x = x / Screen.TwipsPerPixelX
  SendMessageByNum oList.hwnd, LB_SETHORIZONTALEXTENT, x, 0
End Sub
